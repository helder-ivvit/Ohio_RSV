    ...
    _FlagDataCobranzasSoles, _FlagDataCobranzasDolares, _FlagDataEndoso, _activo)
{
    ...
    if (_activo == 1) {
        ...
        var _opt_vida_univ_moce = '<a href="#" class="dropdown-item" onclick="CalcularVUMoce(\'' + _anio + '\',\'' + _mes + '\',\'' + _nombre_mes + '\')"><i class="fas fa-cogs"></i>&nbsp;&nbsp;&nbsp;Procesar Reserva Matem&aacute;tica MOCE</a>';
...
function CalcularVUMoce(_anio, _mes, _nombremes) {
    ...
    $("#titulo_modal_moce_vu").html('Procesar Reserva Matemática V.U. <b style="color: black;">&nbsp;&nbsp;&nbsp;&nbsp;(' + _nombremes + '-' + _anio + ')</b>');

    $("#v_anio_moce_vu").val(_anio);
    $("#v_mes_moce_vu").val(_mes);

    $("#v_moneda_moce_vu").val("");
    $("#v_moneda_moce_vu").change();
...
function GuardarMoceVU() {
    var _moneda = $("#v_moneda_moce_vu").val();
    ...
    $("#modal-sm-procesar_vu_moce").modal('hide');
    $('#modal-loading').modal('show');

    var formData = $('#frmProcesaMoceVU').serialize();
    //console.log(formData);
    //alert("123");
    $.ajax({
        url: "/Universal/ProcesaMoceVU",
        type: "POST",
        data: formData,
        contentType: 'application/x-www-form-urlencoded; charset=UTF-8', // optional
        success: function (data) {

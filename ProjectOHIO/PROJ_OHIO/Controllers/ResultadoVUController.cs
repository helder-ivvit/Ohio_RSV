...
{
    public class ResultadoVUController : Controller
    {
        // GET: ResultadoVU
        public ActionResult Index(int anio, int mes)
        {
            if (Session["UserID"] != null)
            {
                ...
                ViewData["busqueda_anio_vu"] = anio;
                ViewData["busqueda_mes_vu"] = mes;
        ...
        public ActionResult ListarResultado(int anio, int mes, string moneda)
        {
            ...
            nDT_Parametros = nObj.Obtener_Listado("SP_RESULTADO_RESERVA_MATEMATICA_VIDA_UNIVERSAL", anio, mes, moneda).Tables[0];
        ...
        public ActionResult ExportarResultadoReserva(int anio, int mes, string moneda, string poliza)
        {
            ...
            ModeloNegocio.MyNegocio nObj = new ModeloNegocio.MyNegocio();
            DataTable nDT_Obj;
            nDT_Obj = nObj.Obtener_Listado("SP_RESULTADO_RESERVA_MATEMATICA_VIDA_UNIVERSAL", anio, mes, moneda).Tables[0];

            string handle = Guid.NewGuid().ToString();
            byte[] bytesStream = _excel.ExportarResultadoReservaMatematicaVU(nDT_Obj);
            ...
            if (moneda == "S")
                ...
                mone = "Dolares";
            jsonObject.Data = new { FileGuid = handle, FileName = "Resultado_Reserva_Matematica_VU  " + anio.ToString() + "-" + mes.ToString() + "  " + mone + "  " + DateTime.Now.ToString() + ".xlsx" };
            return jsonObject;
        }
        ...
        public ActionResult ExportarDetalleReserva(int anio, int mes, string moneda, string poliza)
        {
            ...
            ModeloNegocio.MyNegocio nObj = new ModeloNegocio.MyNegocio();
            DataTable nDT_Obj;
            nDT_Obj = nObj.Obtener_Listado("SP_DETALLE_RESERVA_MATEMATICA_VIDA_UNIVERSAL", anio, mes, moneda, poliza).Tables[0];

            string handle = Guid.NewGuid().ToString();
            byte[] bytesStream = _excel.ExportarDetalleReservaMatematicaVU(nDT_Obj);
            TempData[handle] = bytesStream.ToArray();
            ...
            if (moneda == "S")
                ...
                mone = "Dolares";
            jsonObject.Data = new { FileGuid = handle, FileName = "Detalle_Reserva_Matematica_VU  " + anio.ToString() + "-" + mes.ToString() + "  " + mone + "  " + DateTime.Now.ToString() + ".xlsx" };
            jsonObject.JsonRequestBehavior = JsonRequestBehavior.AllowGet;
